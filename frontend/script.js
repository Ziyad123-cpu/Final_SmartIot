...
const chart = new Chart(ctx, {
    ...
    data: {
        labels: [],
        datasets: [
            { label: "Kelembapan Tanah (%)", borderColor: "#00e0b8", backgroundColor: "#00e0b833", data: [], fill: true, tension: 0.3, yAxisID: "y" },
            { label: "Suhu Tanah (°C)", borderColor: "#007bff", data: [], fill: false, tension: 0.3, yAxisID: "y1" },
            { label: "Suhu Udara (°C)", borderColor: "#ff9800", data: [], fill: false, tension: 0.3, yAxisID: "y1" },
            { label: "Kelembapan Udara (%)", borderColor: "#e91e63", backgroundColor: "#e91e6333", data: [], fill: true, tension: 0.3, yAxisID: "y" },
        ],
    },
    options: {
        responsive: true,
        plugins: { legend: { labels: { color: "#e8f1f2" } } },
        scales: {
            x: { ticks: { color: "#9bb0bf" }, title: { display: true, text: "Waktu", color: "#9bb0bf" } },
            y: { type: "linear", position: "left", ticks: { color: "#9bb0bf" }, title: { display: true, text: "Kelembapan (%)", color: "#9bb0bf" } },
            y1: { type: "linear", position: "right", ticks: { color: "#9bb0bf" }, title: { display: true, text: "Suhu (°C)", color: "#9bb0bf" }, grid: { drawOnChartArea: false } },
        },
    },
});
...
function updateSystemKPIs(moisturePercent) {
    ...
    if (moisturePercent > BATAS_KERING_TANAH) waterSaved += 0.05;
...
function getFormattedDate() {
    ...
    return { hari: hariList[now.getDay()], tanggal: now.getDate(), bulan: bulanList[now.getMonth()], tahun: now.getFullYear() };
}
...
async function updateWeather() {
    try {
        const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${WEATHER_CITY}&units=metric&appid=${WEATHER_API_KEY}&lang=id`);
        ...
        document.getElementById("weatherIcon").innerHTML = `<img src="https://openweathermap.org/img/wn/${icon}@2x.png" alt="${desc}" style="width:70px;animation:float 3s ease-in-out infinite;">`;
    } catch (err) {
...
async function fetchData() {
    try {
        ...
        document.getElementById("airTemp").textContent = `${data.suhuUdara} %`;
        document.getElementById("airHum").textContent = `${data.kelembapanUdara}°C`;
        ...
        if (moisturePercent <= MOISTURE_THRESHOLD - 5) moistureCard.classList.add("alert-low");
        else if (moisturePercent <= MOISTURE_THRESHOLD) moistureCard.classList.add("alert-warning");
        ...
        row.innerHTML = `<td>${tanggal} ${bulan}</td><td>${hari}</td><td>${waktu}</td><td>${moisturePercent}%</td><td>${data.soilTemperature}°C</td><td>${data.suhuUdara}°C</td><td>${data.kelembapanUdara}%</td><td>${pumpState}</td>`;
        tbody.prepend(row);
...
btnDashboard.addEventListener("click", () => {
    btnDashboard.classList.add("active"); btnDatabase.classList.remove("active");
    dashboardSection.classList.add("active"); databaseSection.classList.remove("active");
    closeSidebar();
});
btnDatabase.addEventListener("click", () => {
    btnDatabase.classList.add("active"); btnDashboard.classList.remove("active");
    databaseSection.classList.add("active"); dashboardSection.classList.remove("active");
    closeSidebar();
});
...
// === Real-time sync tema jika localStorage berubah di tab lain ===
